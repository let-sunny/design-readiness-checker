...
Compare AI-generated code against the original Figma design at the pixel level.
...
```json
{
  ...
  "width": 800,
  "height": 600,
  "figmaScreenshot": "/tmp/canicode-visual-compare/figma.png",
  "codeScreenshot": "/tmp/canicode-visual-compare/code.png",
  "diff": "/tmp/canicode-visual-compare/diff.png"
}
```
...
2. Reads the screenshot dimensions
3. Renders your HTML with Playwright at the same viewport size
4. Compares pixel-by-pixel with pixelmatch (threshold: 0.1)
5. Returns similarity percentage and diff image

### Viewport Sizing

By default, the Playwright viewport matches the Figma screenshot dimensions — no manual sizing needed. Use `--width`/`--height` to override if your HTML has a different intended viewport.

### Requirements

- Playwright with Chromium: `npx playwright install chromium`
- Figma API token with read access to the file
